...
class PhaseKing:
    ...
    def _round1(self, value: str, phase: int, no_of_correct_processes: int):
        ...
        while i < N:
            ...
            if not self._group_view.check_if_server_is_suspended(sender_id):
                if message.header == "Phase King: Message":
                    i = self.__handle_round1_phaseking_msg(data, sender_ids, values, phase, i)

                elif message.header == "View: Suspect":
                    N = self.__handle_round1_suspect_msg(
                        data,
                        suspected_servers,
                        sender_ids,
                        values,
        ...
        self.__debug(
            'PhaseKing ({}Phase {} - Round 1): Results "{}" from "{}"'.format(
                self._topic + "; " if self._topic is not None else "", phase, values, sender_ids
            )
        )
        ...
        self.__debug(
            'PhaseKing ({}Phase {} - Round 1): New value "{}" received from "{}" processes'.format(
                self._topic + "; " if self._topic is not None else "", phase, majority_value, majority_count
            )
        )

        return majority_value, majority_count

    def __handle_round1_phaseking_msg(self, data, sender_ids, values, phase, i):
    ...
    def __handle_round1_suspect_msg(
                    ...
                    if not self._group_view.check_if_server_is_suspended(suspect_msg.identifier):
                        ...
                        self.__debug(
                            'PhaseKing ({}Phase {} - Round 1): Suspending "{}"'.format(
                                self._topic + "; " if self._topic is not None else "",
                                phase,
        ...
        while tiebreaker is None:
            ...
            self.__debug(
                'PhaseKing ({}Phase {} - Round 2): Waiting for "{}"'.format(
                    self._topic + "; " if self._topic is not None else "", phase, phase_king
                )
            )
