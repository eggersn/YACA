...
def normal_test(i):
    ...
    phase_king = MaxPhaseKing(consensus_channel, reliable_multicast, group_view, config, verbose=True)

    value = int(np.random.randint(0, 100, size=i+1)[-1])
    print(value)
...
def topic_test(i):
    def topic_thread(value, topic):
        phase_king = MaxPhaseKing(consensus_channel, reliable_multicast, group_view, config, topic, True)
        value = phase_king.consensus(value)
        print("OUTPUT", value)
    ...
    topic1_thread = threading.Thread(target=topic_thread, args=(np.random.randint(0, 100),"topic1",))
    topic1_thread.start()

    topic2_thread = threading.Thread(target=topic_thread, args=(np.random.randint(0, 100),"topic2",))
    topic2_thread.start()
