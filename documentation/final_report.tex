...
\begin{enumerate}
    ...
    \item[4.] \textit{Fault Tolerance:} We aim to provide Byzantine fault-tolerance. We describe in \Cref{sec:implementation} how the individual building blocks of our implementation are realized to ensure Byzantine fault-tolerance.
\end{enumerate}
...
\begin{enumerate}
    \item[i)] \textit{Suspension:}\footnote{For the sake of full transparency, we note that, at the time of writing, this feature is not implemented yet. Instead, we suspend a server directly after receiving $n-f$ suspect messages as described in \Cref{sec:suspicion}. For a single CO-R-multicast instance, this approach is sufficient since both phase-king algorithm and ISIS algorithm are highly synchronized. In contrast to this, there remains a small probability that, for multiple CO-R-instances, this causes race-conditions and leads to inconsistent behavior. \label{footnote:suspension}} We extend the suspicion protocol described in \Cref{sec:suspicion}. Furthermore, our servers listen to multiple multicast instances (i.e. one for TO-R-multicast and two for CO-R-multicast), which is also considered in the following: 
    \begin{itemize}
